...
interface CarrerasData {
  frontmatter: CarrerasFrontmatter;
  content: string;
}

async function getCarrerasContent(locale: string): Promise<CarrerasData | null> {
  try {
    const filePath = path.join(process.cwd(), 'content', 'carreras', `${locale}.md`);
    
    if (!fs.existsSync(filePath)) {
      // Fallback to Spanish if locale file doesn't exist
      const fallbackPath = path.join(process.cwd(), 'content', 'carreras', 'es.md');
      if (!fs.existsSync(fallbackPath)) {
        return null;
      }
      const fallbackContent = fs.readFileSync(fallbackPath, 'utf8');
      const { data, content } = matter(fallbackContent);
      return { frontmatter: data as CarrerasFrontmatter, content };
    }
    ...
    console.error('Error reading carreras content:', error);
    return null;
  }
}
...
export async function generateMetadata({ 
  ...
  const carrerasData = await getCarrerasContent(locale);
  ...
  return {
    ...
    openGraph: {
      ...
      description: carrerasData.frontmatter.description,
      images: carrerasData.frontmatter.heroImage ? [carrerasData.frontmatter.heroImage] : [],
    }
  };
}
...
export default async function CarrerasPage({ 
  ...
  const { frontmatter, content } = carrerasData;
          ...
          {frontmatter.benefits && frontmatter.benefits.length > 0 && (
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              {frontmatter.benefits.map((benefit, index) => {
                const icons = [Briefcase, Users, Star, Target];
      ...
      <section className="py-16 bg-slate-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-bold text-white mb-6">
            {locale === 'es' ? '¿Listo para unirte a nuestro equipo?' : 
             locale === 'en' ? 'Ready to join our team?' : 
